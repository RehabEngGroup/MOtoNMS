...
% number of Force channels --> related to the force plate type (6 for type 1 and 2 but 8 for type 3)
 ...
 if nFchannels > 0  %if FPdata are present
     ...
     for i = 1 : nFchannels,
         ...
         fchannel= fchannels.info.values(i);
     ...
     for n=1:numberForcePlatform
         ...
         Fv=Values(:,dim1*(n-1)+1:dim1*n); 
         %Values stored force data from all FPs in order
         ...
         if forceplates(n).type == 4
             ...
             if dim1==dim2 && isempty(forceplatesInfo(n).cal_matrix)==0
                 ...
                 disp(['Wrong or empty force plate calibration ' num2str(n) ' matrix: unable to convert force data from V to N (Type 4). Be aware that FPdata stored in the sessionData folder are in V!'])
             end
             ...
             if dim1==dim2 && dim2==length(fieldnames(forceplates(n).channels))
                 Fc=[Fc Fv];
             else
                 disp(['Dimensions of force plate ' num2str(n) ' calibration matrix do not correspond to the number of its output channels: check force plate data and calibration matrix in your C3D file!'])
                 Fc=Values;
             end
         end          
     end
